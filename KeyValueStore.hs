...
twoWithSameKeyHelper ((k1,v1):(k2,v2):kvs) keep  = Nothing -- Complete recursive case for Problem 3, part (a).
twoWithSameKeyHelper _                     _     = Nothing

twoWithSameKey :: (Ord a, Ord b, Eq a) => [(a,b)] -> Maybe (a, b, b, [(a,b)])
twoWithSameKey kvs = Nothing -- Complete for Problem 3, part (b).

suffix :: [([a],b)] -> [([a],b)]
suffix kvs = [] -- Complete for Problem 3, part (c).
